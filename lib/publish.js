...
module.exports = async (npmrc, {npmPublish, pkgRoot}, pkg, context) => {
  ...
  if (npmPublish !== false && pkg.private !== true) {
    ...
    logger.log(`Publishing ${pkg.name} version ${version} to npm registry on dist-tag ${distTag}`);
    const result = execa(
      'npm',
  ...
  logger.log(
    `Skip publishing ${pkg.name} to npm registry as ${npmPublish === false ? 'npmPublish' : "package.json's private property"} is ${
      npmPublish !== false
    }`
  );
