...
module.exports = async (npmrc, {tarballDir, pkgRoot, noWorkspacesUpdate}, {cwd, env, stdout, stderr, nextRelease: {version}, logger}) => {
  ...
  const versionResult = execa(
    ...
    ["version", version, "--userconfig", npmrc, "--no-git-tag-version", "--allow-same-version"].concat(
      noWorkspacesUpdate ? ["--no-workspaces-update"] : ""
    ),
    {
      cwd: basePath,
      env,
